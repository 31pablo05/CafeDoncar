...
import { MenuSkeletonGrid } from './ui/MenuCardSkeleton'
import { useLoading } from '../hooks/useLoading'

export default function MenuList({ items = [], onAddToCart, isLoading: externalLoading = false }) {
  const [showAll, setShowAll] = useState(false)
  const itemsToShow = 6
  const [internalLoading] = useLoading(true, 800) // Loading interno de 800ms
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="relative space-y-8 sm:space-y-10 lg:space-y-12 py-6 sm:py-8 lg:py-12" style={{ zIndex: 1 }}>
        
        {/* Mostrar skeleton mientras carga */}
        {isLoading ? (
          <MenuSkeletonGrid count={itemsToShow} />
        ) : (
          <>
...
// Componente MenuCard extraído para mejor legibilidad
...
// Componente EmptyState extraído
function EmptyState() {
  return (
...
// Componente MoreButton extraído
function MoreButton({ showAll, setShowAll, remainingCount, totalCount, displayedCount }) {
  return (
    <div className="text-center px-4">
      ...
      >
        ...
        <span className="text-lg sm:text-xl relative" style={{ zIndex: 10 }}>
          {showAll ? 'Ver Menos Productos' : `Ver Más Productos (+${remainingCount})`}
        </span>
        {showAll ? (
      ...
      <div className="mt-6 inline-block backdrop-blur-lg bg-white/10 border border-white/20 rounded-full px-6 py-3">
        <p className="text-white/80 text-sm font-medium">
          Mostrando <span className="text-amber-400 font-bold">{displayedCount}</span> de <span className="text-amber-400 font-bold">{totalCount}</span> productos
        </p>
      </div>
    </div>
  );
